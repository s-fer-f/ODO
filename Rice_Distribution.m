...
h = sqrt(K/(K + 1)).*ones(1, Nsim) + 1/sqrt(K + 1).*(randn(1, Nsim) + 1i.*randn(1, Nsim))/sqrt(2);

SNR = ((abs(h).^2)'*Omega)';             % Realizations of instantaneous SNR
...
oPtheo = cdfSNRRiceTh(W_th./Omega, K, 1);
